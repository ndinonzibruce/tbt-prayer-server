...
// In-memory shared people store (survives restarts via re-sync)
let sharedPeople = [];
let lastSync = null;

app.get('/', (req, res) => res.json({ status: 'TBT Prayer Server running', time: new Date().toISOString(), people: sharedPeople.length }));

// --- Shared people endpoints ---
app.get('/people', (req, res) => {
  res.json({ people: sharedPeople, count: sharedPeople.length, lastSync });
});
...
// Update a single person's prayer requests/notes (from any team member)
