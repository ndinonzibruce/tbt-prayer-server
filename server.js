...
app.get('/', (req, res) => res.json({ status: 'TBT Prayer Server v2', time: new Date().toISOString() }));
...
app.get('/cs/lists', async (req, res) => {
  try {
    const r = await fetch(CS_BASE + '/lists?per_page=50', { headers: csH() });
    res.json(await r.json());
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Clearstream subscribers for a specific list using /lists/{id}/subscribers
app.get('/cs/subscribers', async (req, res) => {
  try {
    const { list_id, page = 1 } = req.query;
    let url;
    if (list_id) {
      url = CS_BASE + '/lists/' + list_id + '/subscribers?page=' + page;
    } else {
      url = CS_BASE + '/subscribers?page=' + page;
    }
    const r = await fetch(url, { headers: csH() });
    res.json(await r.json());
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/cs/inbox', async (req, res) => {
  try {
    ...
    const r = await fetch(CS_BASE + '/threads?per_page=' + per_page + '&page=' + page, { headers: csH() });
    res.json(await r.json());
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/pco/lists', async (req, res) => {
  try {
    res.json(await (await fetch(PCO_BASE + '/lists?per_page=50', { headers: pcoH() })).json());
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.listen(PORT, () => console.log('TBT Prayer Server v2 on port ' + PORT));
